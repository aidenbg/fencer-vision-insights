...
serve(async (req)=>{
  ...
  try {
    ...
    if (!video_url || !video_id) {
      console.error(`[${new Date().toISOString()}] Missing required parameters:`, {
        video_url: !!video_url,
        video_id: !!video_id
      });
      return new Response(JSON.stringify({
        error: 'Missing video_url or video_id'
      }), {
        status: 400,
        headers: {
          ...corsHeaders,
    ...
    if (!yoloApiUrl) {
      ...
      return new Response(JSON.stringify({
        error: 'YOLO_MODEL_API_URL not configured'
      }), {
        status: 500,
        headers: {
          ...corsHeaders,
    ...
    console.log(`[${new Date().toISOString()}] YOLO API request payload:`, {
      video_url,
      video_id
    });
    ...
    if (!yoloResponse.ok) {
      ...
      return new Response(JSON.stringify({
        error: `YOLO API failed: ${yoloResponse.status}`,
        details: errorText
      }), {
        status: 500,
        headers: {
    ...
    const { error: updateError, data: updatedRecord } = await supabase.from('videos').update(updateData).eq('id', video_id).select();
